...
leftToRightTransition aut1 aut2 q c =
  if nextAccepting aut1 q c
...
thenA aut1 aut2 =
  ...
  { states = (Left <$> states aut1) ++ (Right <$> states aut2)
  , initStates =
      (Left <$> initStates aut1) ++
      (if hasAcceptingInitState aut1
         then Right <$> initStates aut2
         else [])
  , isAccepting =
      either
        (if hasAcceptingInitState aut2
           then isAccepting aut1
           else const False)
        (isAccepting aut2)
  , transition =
      either
        (concatTransition ((fmap Left .) . transition aut1) (leftToRightTransition aut1 aut2))
        ((fmap Right .) . transition aut2)
  }
